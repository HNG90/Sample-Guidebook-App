...
""",
    ...
    func getFileData() -> [City]{
        ...
        //Get file path the JSON data
        if let url = Bundle.main.url(forResource: "DemoData", withExtension: "json"){
            
            do {
                //Read the file and decode
                let data = try Data(contentsOf: url) //throws error possibly
                
                //Parse into Swift language
                let decoder = JSONDecoder()
                
                do{
                    let cities = try decoder.decode([City].self, from: data) //may throw error
                    return cities
                }
                catch{
                    print("couldnt parse the json: \(error)") //handle the error
                }
            }
            catch{
                print("couldnt read the file: \(error)")
            }
